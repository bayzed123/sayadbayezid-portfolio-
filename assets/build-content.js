...
const CONTENT_TYPES = [
  ...
  { dir: "blog", label: "Blog", indexTitle: "Blog", indexPage: "blog.html" },
];
...
function buildContentType(type) {
  ...
  const outDir = path.join(ROOT, "site", type.dir);
  ...
  fs.writeFileSync(path.join(ROOT, "site", type.indexPage), renderIndexPage(type, entries));
...
console.log("\nDone. Generated pages are under site/ — copy that into your site repo alongside the existing pages.");
  ...
  build:
    ...
    steps:
      ...
      - run: |
          ...
          git add case-studies news blog case-studies.html news.html blog.html
          git commit -m "Regenerate content" || echo "Nothing to commit"
          git push
*/
